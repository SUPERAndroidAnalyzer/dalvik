    ...
    fn from_reader<R: Read>(reader: &mut R) -> Result<Value> {
        ...
        match value_type {
            ...
            VALUE_SHORT => match arg {
                0 => Ok(Value::Short(
                    reader.read_i8().chain_err(|| "could not read Short")? as i16,
                )),
