...
use super::{Visibility, Value, Annotation, EncodedAnnotation};
use super::{AnnotationElement, Array, AccessFlags};
...
impl PrototypeIdData {
    ...
    pub fn from_reader<R: Read + Seek, B: ByteOrder>(reader: &mut R) -> Result<PrototypeIdData> {
        let shorty_index = reader.read_u32::<B>().chain_err(
            || "could not read the sorty_index field",
        )?;
        let return_type_index = reader.read_u32::<B>().chain_err(
            || "could not read the return_type_index field",
        )?;
        let parameters_offset = reader.read_u32::<B>().chain_err(
            || "could not read the parameters_offset field",
        )?;
        Ok(PrototypeIdData {
...
impl FieldIdData {
    ...
    pub fn from_reader<R: Read, B: ByteOrder>(reader: &mut R) -> Result<FieldIdData> {
        let class_index = reader.read_u16::<B>().chain_err(
            || "could not read the class_index field",
        )?;
        let type_index = reader.read_u16::<B>().chain_err(
            || "could not read the type_index field",
        )?;
        let name_index = reader.read_u32::<B>().chain_err(
            || "could not read the name_index field",
        )?;
        Ok(FieldIdData {
...
impl MethodIdData {
    ...
    pub fn from_reader<R: Read, B: ByteOrder>(reader: &mut R) -> Result<MethodIdData> {
        ...
        let prototype_index = reader.read_u16::<B>().chain_err(
            || "could not read the prototype_index field",
        )?;
        let name_index = reader.read_u32::<B>().chain_err(
            || "could not read the name_index field",
        )?;
...
impl ClassDefData {
    ...
    pub fn from_reader<R: Read, B: ByteOrder>(reader: &mut R) -> Result<ClassDefData> {
        ...
        let class_index = reader.read_u32::<B>().chain_err(
            || "could not read the class_index field",
        )?;
        let access_flags = reader.read_u32::<B>().chain_err(
            || "could not read the access_flags field",
        )?;
        let superclass_index = reader.read_u32::<B>().chain_err(
            || "could not read the superclass_id field",
        )?;
        let interfaces_offset = reader.read_u32::<B>().chain_err(
            || "could not read the interfaces_offset field",
        )?;
        let source_file_index = reader.read_u32::<B>().chain_err(
            || "could not read the source_file_id field",
        )?;
        let annotations_offset = reader.read_u32::<B>().chain_err(
            || "could not read the annotations_offset field",
        )?;
        let class_data_offset = reader.read_u32::<B>().chain_err(
            || "could not read the class_data_offset field",
        )?;
        let static_values_offset = reader.read_u32::<B>().chain_err(
            || "could not read the static_values_offset field",
        )?;
...
impl Value {
    fn from_reader<R: Read>(reader: &mut R) -> Result<Value> {
        ...
        reader.read_exact(&mut value_type).chain_err(
            || "could not read the value_type field",
        )?;
        ...
        match value_type {
            VALUE_BYTE => {
                if arg == 0 {
                    Ok(Value::Byte(
                        reader.read_i8().chain_err(|| "could not read Byte")?,
                    ))
                } else {
                    Err(
            ...
            VALUE_SHORT => {
                match arg {
                    0 => {
                        Ok(Value::Short(
                            reader.read_i8().chain_err(|| "could not read Short")? as
                                i16,
                        ))
                    }
                    1 => {
                        Ok(Value::Short(reader.read_i16::<LittleEndian>().chain_err(
                            || "could not read Short",
                        )?))
                    }
                    a => {
                        Err(
                            ErrorKind::InvalidValue(format!("invalid arg ({}) for Short value", a))
                                .into(),
                        )
                    }
                }
            ...
            VALUE_CHAR => {
                match arg {
                    0 => {
                        Ok(Value::Char(
                            reader.read_u8().chain_err(|| "could not read Char")? as u16,
                        ))
                    }
                    1 => {
                        Ok(Value::Char(reader.read_u16::<LittleEndian>().chain_err(
                            || "could not read Char",
                        )?))
                    }
                    a => {
                        Err(
                            ErrorKind::InvalidValue(format!("invalid arg ({}) for Char value", a))
                                .into(),
                        )
                    }
                }
            ...
            VALUE_INT => {
                match arg {
                    0 => {
                        Ok(Value::Int(
                            reader.read_i8().chain_err(|| "could not read Int")? as i32,
                        ))
                    }
                    1 => {
                        Ok(Value::Int(reader.read_i16::<LittleEndian>().chain_err(
                            || "could not read Int",
                        )? as i32))
                    }
                    2 => {
                        let mut bytes = [0_u8; 3];
                        reader.read_exact(&mut bytes).chain_err(
                            || "could not read Int",
                        )?;
                    ...
                    3 => {
                        Ok(Value::Int(reader.read_i32::<LittleEndian>().chain_err(
                            || "could not read Int",
                        )?))
                    }
                    a => {
            ...
            VALUE_LONG => {
                match arg {
                    ...
                    1 => {
                        Ok(Value::Long(reader.read_i16::<LittleEndian>().chain_err(
                            || "could not read Long",
                        )? as i64))
                    }
                    2 => {
                    ...
                    3 => {
                        Ok(Value::Long(reader.read_i32::<LittleEndian>().chain_err(
                            || "could not read Long",
                        )? as i64))
                    }
                    4 => {
                        ...
                        // Reading in little endian
                        Ok(Value::Long(
                            bytes[0] as i64 | (bytes[1] as i64) << 8 | (bytes[2] as i64) << 16 |
                                (bytes[3] as i64) << 24 |
                    ...
                    7 => {
                        Ok(Value::Long(reader.read_i64::<LittleEndian>().chain_err(
                            || "could not read Long",
                        )?))
                    }
                    _ => unreachable!(),
            ...
            VALUE_FLOAT => {
                match arg {
                    c @ 0...3 => {
                        ...
                        reader.read_exact(&mut bytes[..c as usize + 1]).chain_err(
                    ...
                    a => {
                        Err(
                            ErrorKind::InvalidValue(format!("invalid arg ({}) for Float value", a))
                                .into(),
                        )
                    }
                }
            ...
            VALUE_STRING => {
                let string_index = Value::read_u32(reader, arg).chain_err(
                    || "could not read String index",
                )?;
                Ok(Value::String(string_index))
            }
            VALUE_TYPE => {
                let type_index = Value::read_u32(reader, arg).chain_err(
                    || "could not read Type index",
                )?;
                Ok(Value::Type(type_index))
            }
            VALUE_FIELD => {
                let field_index = Value::read_u32(reader, arg).chain_err(
                    || "could not read Field index",
                )?;
                Ok(Value::Field(field_index))
            }
            VALUE_METHOD => {
                let method_index = Value::read_u32(reader, arg).chain_err(
                    || "could not read Method index",
                )?;
                Ok(Value::Method(method_index))
            }
            VALUE_ENUM => {
                let enum_index = Value::read_u32(reader, arg).chain_err(
                    || "could not read Enum index",
                )?;
                Ok(Value::Enum(enum_index))
            }
            VALUE_ARRAY => {
                let array = Array::from_reader(reader).chain_err(
                    || "could not read Array",
                )?;
                Ok(Value::Array(array))
            }
            VALUE_ANNOTATION => {
                let annotation = EncodedAnnotation::from_reader(reader).chain_err(
                    || "could not read Annotation value",
                )?;
                    ...
                    _ => {
                        Err(
                            ErrorKind::InvalidValue(
                                format!("invalid arg ({}) for Boolean value", arg),
                            ).into(),
                        )
                    }
                }
            ...
            v => Err(
                ErrorKind::InvalidValue(format!("invalid value type {:#04x}", v)).into(),
            ),
        }
    }
    ...
    fn read_u32<R: Read>(reader: &mut R, arg: u8) -> Result<u32> {
        match arg {
            ...
            a => Err(
                ErrorKind::InvalidValue(format!("invalid arg ({}) for u32 value", a)).into(),
            ),
        }
    }
}
...
impl Array {
    ...
    pub fn from_reader<R: Read>(reader: &mut R) -> Result<Array> {
        let (size, _) = read_uleb128(reader).chain_err(
            || "could not read array size",
        )?;
        let mut array = Vec::with_capacity(size as usize);
        for _ in 0..size {
            let value = Value::from_reader(reader).chain_err(
...
impl EncodedAnnotation {
    ...
    pub fn from_reader<R: Read>(reader: &mut R) -> Result<EncodedAnnotation> {
        ...
        for _ in 0..size {
            let (name_id, _) = read_uleb128(reader).chain_err(
                || "could not read element's name_id",
            )?;
            let value = Value::from_reader(reader).chain_err(
                || "could not read element's value",
            )?;
...
impl Annotation {
    ...
    pub fn from_reader<R: Read>(reader: &mut R) -> Result<Annotation> {
        ...
        reader.read_exact(&mut visibility).chain_err(
            || "could not read visibility",
        )?;
...
impl AnnotationsDirectoryOffsets {
    ...
    pub fn from_reader<R: Read, B: ByteOrder>(
        ...
        let class_annotations_offset = reader.read_u32::<B>().chain_err(
            || "could not read class annotations offset",
        )?;
        let field_annotations_size = reader.read_u32::<B>().chain_err(
            || "could not read field annotations size",
        )? as usize;
        let method_annotations_size = reader.read_u32::<B>().chain_err(
            || "could not read method annotations size",
        )? as usize;
        let parameter_annotations_size = reader.read_u32::<B>().chain_err(
            || "could not read parameter annotations size",
        )? as usize;

        let mut field_annotations = Vec::with_capacity(field_annotations_size);
        for _ in 0..field_annotations_size {
            let field_id = reader.read_u32::<B>().chain_err(
                || "could not read field ID for field annotation",
            )?;
            let offset = reader.read_u32::<B>().chain_err(
                || "could not read field annotation offset",
            )?;
        ...
        for _ in 0..method_annotations_size {
            let method_id = reader.read_u32::<B>().chain_err(
                || "could not read method ID for method annotation",
            )?;
            let offset = reader.read_u32::<B>().chain_err(
                || "could not read method annotation offset",
            )?;
        ...
        for _ in 0..parameter_annotations_size {
            let method_id = reader.read_u32::<B>().chain_err(
                || "could not read method ID for parameter annotation",
            )?;
            let offset = reader.read_u32::<B>().chain_err(
                || "could not read annotation offset",
            )?;
...
impl ClassData {
    ...
    pub fn from_reader<R: Read>(reader: &mut R) -> Result<ClassData> {
        let (static_fields_size, _) = read_uleb128(reader).chain_err(
            || "could not read static_fields_size field",
        )?;
        let (instance_fields_size, _) = read_uleb128(reader).chain_err(
            || "could not read instance_fields_size field",
        )?;
        let (direct_methods_size, _) = read_uleb128(reader).chain_err(
            || "could not read direct_methods_size field",
        )?;
        let (virtual_methods_size, _) = read_uleb128(reader).chain_err(
            || "could not read virtual_methods_size field",
        )?;
        ...
        ClassData::read_methods(reader, direct_methods_size, &mut direct_methods)
            .chain_err(|| "could not read class direct methods")?;

        let mut virtual_methods = Vec::with_capacity(virtual_methods_size as usize);
        ClassData::read_methods(reader, virtual_methods_size, &mut virtual_methods)
    ...
    fn read_fields<R: Read>(
        ...
        if field_count > 0 {
            ...
            let (access_flags, _) = read_uleb128(reader).chain_err(
                || "could not read field access flags",
            )?;
    ...
    fn read_methods<R: Read>(
        ...
        if method_count > 0 {
            ...
            let (method_id, _) = read_uleb128(reader).chain_err(
                || "could not read method ID",
            )?;
            let (access_flags, _) = read_uleb128(reader).chain_err(
                || "could not read method access flags",
            )?;
            let (code_offset, _) = read_uleb128(reader).chain_err(
                || "could not read method code offset",
            )?;
            ...
            for _ in 1..method_count {
                let (method_id_diff, _) = read_uleb128(reader).chain_err(
                    || "could not read method ID",
                )?;
                let (access_flags, _) = read_uleb128(reader).chain_err(
                    || "could not read method access flags",
...
impl DebugInfo {
    ...
    pub fn from_reader<R: Read>(reader: &mut R) -> Result<(DebugInfo, u32)> {
        let (line_start, mut read) = read_uleb128(reader).chain_err(
            || "could not read line_start field",
        )?;
        let (parameters_size, read_p) = read_uleb128(reader).chain_err(
            || "could not read parameters_size field",
        )?;
        read += read_p;

        let mut parameter_names = Vec::with_capacity(parameters_size as usize);
        for _ in 0..parameters_size {
            let (name_index, read_i) = read_uleb128p1(reader).chain_err(
                || "could not read parameter name index",
            )?;
            read += read_i;
            parameter_names.push(name_index);
        }

        let (bytecode, read_b) = DebugBytecode::from_reader(reader).chain_err(
            || "could not read debug bytecode",
        )?;
        read += read_b;
...
impl DebugBytecode {
    ...
    fn from_reader<R: Read>(reader: &mut R) -> Result<(DebugBytecode, u32)> {
        ...
        let mut read = 0;
        loop {
            let (instruction, read_i) = DebugInstruction::from_reader(reader).chain_err(
                || "could not read instruction",
            )?;
            read += read_i;
...
impl DebugInstruction {
    fn from_reader<R: Read>(reader: &mut R) -> Result<(DebugInstruction, u32)> {
        let mut opcode = [0_u8];
        reader.read_exact(&mut opcode).chain_err(
...
impl CodeItem {
    ...
    pub fn from_reader<R: Read, B: ByteOrder>(reader: &mut R) -> Result<CodeItem> {
        let registers_size = reader.read_u16::<B>().chain_err(
            || "could not read registers size",
        )?;
        let ins_size = reader.read_u16::<B>().chain_err(
            || "could not read incoming words size",
        )?;
        let outs_size = reader.read_u16::<B>().chain_err(
            || "could not read outgoing words size",
        )?;
        let tries_size = reader.read_u16::<B>().chain_err(
            || "could not read tries size",
        )?;
        let debug_info_offset = reader.read_u32::<B>().chain_err(
            || "could not read debug information offset",
        )?;
        let insns_size = reader.read_u32::<B>().chain_err(
            || "could not read the size of the bytecode array",
        )?;

        let mut insns = Vec::with_capacity(insns_size as usize);
        for _ in 0..insns_size {
            insns.push(reader.read_u16::<B>().chain_err(
                || "could not read bytecode",
            )?);
        }

        if tries_size != 0 && (insns_size & 0b1 != 0) {
            let mut padding = [0_u8; 2];
            reader.read_exact(&mut padding).chain_err(
                || "could not read padding before tries",
            )?;
        }
        ...
        if tries_size > 0 {
            let (handlers_size, _) = read_uleb128(reader).chain_err(
                || "could not read catch handlers size",
            )?;

            handlers.reserve_exact(handlers_size as usize);
            for _ in 0..handlers_size {
                let (handler, _) = CatchHandler::from_reader(reader).chain_err(
                    || "could not read catch handler",
                )?;
...
impl TryItem {
    ...
    fn from_reader<R: Read, B: ByteOrder>(reader: &mut R) -> Result<TryItem> {
        let start_address = reader.read_u32::<B>().chain_err(
            || "could not read start address",
        )?;
        let insn_count = reader.read_u16::<B>().chain_err(
            || "could not read instruction count",
        )?;
        let handler_offset = reader.read_u16::<B>().chain_err(
            || "could not read catch handler offset",
        )?;
...
impl CatchHandler {
    ...
    fn from_reader<R: Read>(reader: &mut R) -> Result<(CatchHandler, u32)> {
        let (size, mut read) = read_sleb128(reader).chain_err(
            || "could not read the catch handler size",
        )?;
        ...
        let catch_all_addr = if size < 1 {
            let (addr, read_ca) = read_uleb128(reader).chain_err(
                || "could not read the catch all address",
            )?;
            read += read_ca;
            Some(addr)
        } else {
